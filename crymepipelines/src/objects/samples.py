...
class SamplesManager:
    ...
    def _add_samples(self, date, n_samples=500):
        min_lat, max_lat, min_long, max_long = self.gd.get_bounding_box()
        valid_samples = self._generate_location_times(min_lat, max_lat, min_long, max_long, date, n_samples)
        self._save_samples(valid_samples)

    def _generate_location_times(self, min_lat, max_lat, min_long, max_long, date, n_samples):
        samples = []
        length = 0
