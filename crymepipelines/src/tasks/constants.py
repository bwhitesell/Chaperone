from lightgbm import LGBMClassifier

crime_groups = {
    'AB': 'Assault / Battery',
    'MIO': 'Misc. Infrequent Offenses:',
    'LTVC': 'Life-Threatening Violent Crime',
    'B': 'Burglary',
    'T': 'Theft',
    'BTV': 'Burglary / Theft (From Vehicle)',
    'R': 'Robbery (Violent Theft)',
    'VBBS': 'Vehicle/Bike/Boat Stolen',
    'PDT': 'Property Damage / Threats',
    'SP': 'Shoplifting/Pickpocket',
}


crime_group_mapping = {
    624: 'AB',
    626: 'AB',
    860: 'AB',
    625: 'AB',
    623: 'AB',
    627: 'AB',
    622: 'AB',
    437: 'MIO:',
    230: 'LTVC',
    236: 'LTVC',
    121: 'LTVC',
    110: 'LTVC',
    251: 'LTVC',
    235: 'LTVC',
    122: 'LTVC',
    231: 'LTVC',
    250: 'LTVC',
    330: 'BTV',
    310: 'B',
    440: 'T',
    420: 'BTV',
    341: 'T',
    350: 'T',
    320: 'B',
    441: 'T',
    450: 'T',
    351: 'T',
    451: 'T',
    470: 'T',
    331: 'BTV',
    421: 'BTV',
    210: 'R',
    220: 'R',
    510: 'VBBS',
    480: 'VBBS',
    520: 'VBBS',
    487: 'VBBS',
    485: 'VBBS',
    740: 'PDT',
    745: 'PDT',
    930: 'PDT',
    888: 'PDT',
    761: 'PDT',
    648: 'PDT',
    763: 'PDT',
    442: 'SP',
    343: 'SP',
    443: 'SP',
    352: 'SP',
    753: 'MIO',
    886: 'MIO',
    940: 'MIO',
    850: 'MIO',
    812: 'MIO',
    815: 'MIO',
    910: 'MIO',
    647: 'MIO',
    820: 'MIO',
    932: 'MIO',
    755: 'MIO',
    920: 'MIO',
    760: 'MIO',
    822: 'MIO',
    813: 'MIO',
    933: 'MIO',
    668: 'MIO',
    943: 'MIO',
    438: 'MIO',
    806: 'MIO',
    922: 'MIO',
    805: 'MIO',
    434: 'MIO'
}


safety_rel_crimes = {
    624: 'BATTERY - SIMPLE ASSAULT',
    740: 'VANDALISM - FELONY ($400 & OVER, ALL CHURCH VANDALISMS)',
    626: 'INTIMATE PARTNER - SIMPLE ASSAULT',
    330: 'BURGLARY FROM VEHICLE',
    510: 'VEHICLE - STOLEN',
    310: 'BURGLARY',
    440: 'THEFT PLAIN - PETTY ($950 & UNDER)',
    230: 'ASSAULT WITH DEADLY WEAPON, AGGRAVATED ASSAULT',
    210: 'ROBBERY',
    420: 'THEFT FROM MOTOR VEHICLE - PETTY ($950 & UNDER)',
    745: 'VANDALISM - MISDEAMEANOR ($399 OR UNDER)',
    930: 'CRIMINAL THREATS - NO WEAPON DISPLAYED',
    341: 'THEFT-GRAND ($950.01 & OVER)EXCPT,GUNS,FOWL,LIVESTK,PROD',
    888: 'TRESPASSING',
    236: 'INTIMATE PARTNER - AGGRAVATED ASSAULT',
    331: 'THEFT FROM MOTOR VEHICLE - GRAND ($400 AND OVER)',
    761: 'BRANDISH WEAPON',
    480: 'BIKE - STOLEN',
    350: 'THEFT, PERSON',
    442: 'SHOPLIFTING - PETTY THEFT ($950 & UNDER)',
    343: 'SHOPLIFTING-GRAND THEFT ($950.01 & OVER)',
    443: 'SHOPLIFTING - ATTEMPT',
    320: 'BURGLARY, ATTEMPTED',
    220: 'ATTEMPTED ROBBERY',
    860: 'BATTERY WITH SEXUAL CONTACT',
    625: 'OTHER ASSAULT',
    623: 'BATTERY POLICE (SIMPLE)',
    121: 'RAPE, FORCIBLE',
    627: 'CHILD ABUSE (PHYSICAL) - SIMPLE ASSAULT',
    753: 'DISCHARGE FIREARMS/SHOTS FIRED',
    886: 'DISTURBING THE PEACE ',
    648: 'ARSON',
    437: 'RESISTING ARREST',
    940: 'EXTORTION',
    520: 'VEHICLE - ATTEMPT STOLEN',
    850: 'INDECENT EXPOSURE',
    812: 'CRM AGNST CHLD (13 OR UNDER) (14-15 & SUSP 10 YRS OLDER)',
    352: 'PICKPOCKET',
    251: 'SHOTS FIRED AT INHABITED DWELLING',
    110: 'CRIMINAL HOMICIDE',
    421: 'THEFT FROM MOTOR VEHICLE - ATTEMPT',
    815: 'SEXUAL PENETRATION W/FOREIGN OBJECT',
    910: 'KIDNAPPING',
    441: 'THEFT PLAIN - ATTEMPT',
    647: 'THROWING OBJECT AT MOVING VEHICLE',
    820: 'ORAL COPULATION',
    932: 'PEEPING TOM',
    763: 'STALKING',
    235: 'CHILD ABUSE (PHYSICAL) - AGGRAVATED ASSAULT',
    122: 'RAPE, ATTEMPTED',
    351: 'PURSE SNATCHING',
    231: 'ASSAULT WITH DEADLY WEAPON ON POLICE OFFICER',
    450: 'THEFT FROM PERSON - ATTEMPT',
    250: 'SHOTS FIRED AT MOVING VEHICLE, TRAIN OR AIRCRAFT',
    755: 'BOMB SCARE',
    920: 'KIDNAPPING - GRAND ATTEMPT',
    760: 'LEWD/LASCIVIOUS ACTS WITH CHILD',
    822: 'HUMAN TRAFFICKING - COMMERCIAL SEX ACTS',
    813: 'CHILD ANNOYING (17YRS & UNDER)',
    933: 'PROWLER',
    668: 'EMBEZZLEMENT, GRAND THEFT ($950.01 & OVER)',
    434: 'FALSE IMPRISONMENT',
    943: 'CRUELTY TO ANIMALS',
    438: 'RECKLESS DRIVING',
    806: 'PANDERING',
    487: 'BOAT - STOLEN',
    622: 'BATTERY ON A FIREFIGHTER',
    451: 'PURSE SNATCHING - ATTEMPT',
    922: 'CHILD STEALING',
    805: 'PIMPING',
    485: 'BIKE - ATTEMPTED STOLEN',
    470: 'TILL TAP - GRAND THEFT ($950.01 & OVER)',
}


cc_hyperparameters = {
    'n_t': {
        'base_estimator': LGBMClassifier(**{
            'max_depth': 3,
            'objective': 'binary',
            'boosting_type': 'gbdt',
            'learning_rate': 0.7226666666666667,
            'num_leaves': 140,
            'n_estimators': 100,
            'reg_lambda': 0.8333333333333333,
            'min_split_gain': 0.0,
            'min_child_samples': 2,
        }),
        'cv': 5,
        'method': 'isotonic',
    },
    'n_b':  {
        'base_estimator': LGBMClassifier(**{
            'max_depth': 2,
            'objective': 'binary',
            'boosting_type': 'gbdt',
            'learning_rate': 0.43400000000000005,
            'num_leaves': 10,
            'n_estimators': 100,
            'reg_lambda': 0.16666666666666666,
            'min_child_samples': 2,
        }),
        'cv': 3,
        'method': 'isotonic',
    },
    'n_ab':  {
        'base_estimator': LGBMClassifier(**{
            'max_depth': 2,
            'objective': 'binary',
            'boosting_type': 'gbdt',
            'learning_rate': 1.0113333333333332,
            'num_leaves': 10, 'n_estimators': 100,
            'reg_lambda': 1.5,
            'min_split_gain': 0,
            'min_child_samples': 2,
        }),
        'cv': 5,
        'method': 'isotonic',
    },
    'n_ltvc': {
        'base_estimator': LGBMClassifier(**{
            'max_depth': 2,
            'objective': 'binary',
            'boosting_type': 'gbdt',
            'learning_rate': 0.43400000000000005,
            'num_leaves': 10,
            'n_estimators': 100,
            'reg_lambda': 1.0,
        }),
        'cv': 10,
        'method': 'isotonic',
    },
    'n_sp': {
        'base_estimator': LGBMClassifier(**{
            'max_depth': 5,
            'objective': 'binary',
            'boosting_type': 'goss',
            'learning_rate': 0.2896666666666667,
            'num_leaves': 80,
            'n_estimators': 100,
            'reg_lambda': 1.1111111111111112,
            'min_child_samples': 226,
            'min_split_gain': 0.07777777777777778,
        }),
        'cv': 10,
        'method': 'isotonic',
    },
    'n_vbbs': {
        'base_estimator': LGBMClassifier(**{
            'max_depth': 2,
            'objective': 'binary',
            'boosting_type': 'gbdt',
            'learning_rate': 0.43400000000000005,
            'num_leaves': 10,
            'n_estimators': 100,
            'reg_lambda': 0,
            'min_split_gain': 0,
            'min_child_samples': 2,
        }),
        'cv': 2,
        'method': 'isotonic',
    },
    'n_pdt': {
        'base_estimator': LGBMClassifier(**{
            'max_depth': 2,
            'objective': 'binary',
            'boosting_type': 'gbdt',
            'learning_rate': 0.8670000000000001,
            'num_leaves': 40,
            'n_estimators': 100,
            'reg_lambda': 1.1111111111111112,
            'min_split_gain': 0,
            'min_child_samples': 2,
        }),
        'cv': 5,
        'method': 'isotonic',
    },
    'n_btv':  {
        'base_estimator': LGBMClassifier(**{
            'max_depth': 2,
            'objective': 'binary',
            'boosting_type': 'gbdt',
            'learning_rate': 0.43400000000000005,
            'num_leaves': 10,
            'n_estimators': 100,
            'reg_lambda': 1.3333333333333333,
            'min_split_gain': 0,
            'min_child_samples': 2,
        }),
        'cv': 5,
        'method': 'isotonic',
    },
    'n_mio': {
        'base_estimator': LGBMClassifier(**{
            'max_depth': 2,
            'objective': 'binary',
            'boosting_type': 'gbdt',
            'learning_rate': 0.2896666666666667,
            'num_leaves': 10,
            'n_estimators': 100,
            'reg_lambda': 0.2222222222222222,
            'min_split_gain': 0,
            'min_child_samples': 2,
        }),
        'cv': 5,
        'method': 'isotonic',
    },
    'n_r': {
        'base_estimator': LGBMClassifier(**{
            'max_depth': 3,
            'objective': 'binary',
            'boosting_type': 'gbdt',
            'learning_rate': 0.43400000000000005,
            'num_leaves': 10,
            'n_estimators': 100,
            'reg_lambda': 2.0,
            'min_split_gain': 0.1,
            'min_child_samples': 2,
        }),
        'cv': 2,
        'method': 'isotonic',
    }
}
